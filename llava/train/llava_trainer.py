...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        """Override to set sync_each_batch=True for DeepSpeed compatibility.

        DeepSpeed ZeRO-2/3 handles gradient accumulation internally and does
        not support the no_sync() context manager that Accelerate uses by
        default.  Setting sync_each_batch=True tells Accelerate to skip
        no_sync() and let DeepSpeed manage accumulation natively.
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """Override compute_loss to pass global_step and collect SGLD losses."""
        ...
        # Accumulate SGLD losses for logging
    ...
    def log(self, logs: Dict[str, float]) -> None:
        """Override log to add SGLD-specific metrics."""
    ...
    def create_optimizer(self):
        """Setup optimizer with separate param groups for projector and SGLD modules."""
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None:
                ...
                # Base parameter groups (excluding projector and SGLD)
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters()
                            if (n in decay_parameters and n not in projector_parameters
                                and n not in sgld_parameters and p.requires_grad)
                        ],
                ...
                # Add SGLD parameter groups
                if sgld_parameters:
                    sgld_lr = sgld_decoder_lr if sgld_decoder_lr is not None else self.args.mm_projector_lr
                ...
                # Standard groups (excluding SGLD)
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters()
                ...
                # Add SGLD parameter groups with optional custom LR
                if sgld_parameters:
                    if sgld_decoder_lr is not None:
                        ...
                        # Include SGLD params in base groups
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            if getattr(self.args, 'sgld_enable', False):
                keys_to_match.extend(['sgld_mask_embed', 'sgld_decoder', 'sgld_tau_embed'])
