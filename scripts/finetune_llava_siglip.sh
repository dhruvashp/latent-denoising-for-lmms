...
# Latent-denoising instruction tuning for SigLIP + Vicuna-7B.
#
# Loads raw Vicuna-7B + the Latent-denoising pretrained projector (SigLIP → Vicuna)
# and trains LLM + projector + denoising decoder + tau embedding; vision tower frozen.
# Saliency: attention column-mean from penultimate layer (no CLS token).
#
...
OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/llava-siglip-vicuna-7b-finetune}"
LOG_DIR="${LOG_DIR:-./logs}"
REQUIRED_GPUS="${REQUIRED_GPUS:-}"
SAVE_STEPS="${SAVE_STEPS:-1298}"
SAVE_TOTAL_LIMIT="${SAVE_TOTAL_LIMIT:-2}"
mkdir -p "${LOG_DIR}"
RUN_LOG="${LOG_DIR}/finetune_llava_siglip_$(date +%Y%m%d_%H%M%S).log"

MODEL_NAME="${MODEL_NAME:-lmsys/vicuna-7b-v1.5}"
PRETRAIN_PROJECTOR="${PRETRAIN_PROJECTOR:-${CHECKPOINT_ROOT}/llava-siglip-vicuna-7b-pretrain/mm_projector.bin}"
...
echo "Launching Latent-denoising fine-tune (LLaVA / SigLIP / Vicuna-7B)"
